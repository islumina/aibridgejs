...
> **Status: 0.5.6.** Core bridge, iframe / flutter adapters, and environment detection are all live; hardened envelope validation. See [CHANGELOG.md](CHANGELOG.md) for release history.
...
const bridge = createBridge({
  ...
  timeoutMs?: number,       // per-call timeout in ms; default 10000.
                            // <= 0 disables the per-call timer entirely.
});
```

A non-positive `timeoutMs` (`0` or negative), passed either here or per call,
**disables** the per-call timeout: the call stays pending until a response, an
`AbortSignal`, `reset()`, or `dispose()` settles it. Use it only when you supply
your own deadline (e.g. an `AbortSignal` with `setTimeout`); against a silent
host an un-timed call pins its pending entry until teardown.
...
- The timeout elapses (default: `options.timeoutMs` ?? constructor `timeoutMs`).
  A `timeoutMs` of `0` or negative disables the timer — the call will not
  self-reject on a deadline; settle it via `AbortSignal`, `reset()`, or
  `dispose()`.
